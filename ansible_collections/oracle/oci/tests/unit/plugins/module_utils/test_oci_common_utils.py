...
@pytest.fixture
def compare_dicts_patch(mocker):
    return mocker.patch.object(oci_common_utils, "compare_dicts")


@pytest.fixture
def compare_lists_patch(mocker):
    return mocker.patch.object(oci_common_utils, "compare_lists")


def test_compare_dicts_when_source_or_target_are_none():
    assert oci_common_utils.compare_dicts(None, None) is False
    assert oci_common_utils.compare_dicts({}, None) is False
    assert oci_common_utils.compare_dicts(None, {}) is False


def test_compare_dicts_when_source_or_target_are_not_dicts():
    assert oci_common_utils.compare_dicts([], {}) is False
    assert oci_common_utils.compare_dicts([], []) is False
    assert oci_common_utils.compare_dicts("testsourcestr", {}) is False
    assert oci_common_utils.compare_dicts(1, {}) is False
    assert oci_common_utils.compare_dicts(True, {}) is False
    assert oci_common_utils.compare_dicts({}, "testtargetstr") is False
    assert oci_common_utils.compare_dicts({}, 1) is False
    assert oci_common_utils.compare_dicts({}, True) is False


def test_compare_dicts_when_source_and_target_are_empty():
    assert oci_common_utils.compare_dicts({}, {}) is True


def test_compare_dicts_when_source_has_more_keys():
    s = {"key1": "val1"}
    t = {}
    assert oci_common_utils.compare_dicts(s, t) is False
    assert (
        oci_common_utils.compare_dicts(s, t, ignore_attr_if_not_in_target=True) is True
    )
...
def test_compare_dicts_when_source_has_less_keys():
    s = {"key1": "val1"}
    t = {"key1": "val1", "key2": "val2"}
    assert oci_common_utils.compare_dicts(s, t) is True
...
def test_compare_dicts_when_dicts_have_list_values():
...
def test_compare_dicts_when_dicts_have_dict_values():
    s = {"key1": "val1", "key2": {"subkey1": "subval1", "subkey2": "subval2"}}
    t = {"key1": "val1", "key2": {"subkey1": "subval1", "subkey2": "subval2"}}
    assert oci_common_utils.compare_dicts(s, t) is True
...
def test_compare_dicts_returns_False_when_dicts_are_different():
...
def test_is_in_list_when_element_is_dict(compare_dicts_patch):
    test_list = [
        {"testkey1": "testval1"},
        {"testkey1": "testval2"},
        {"testkey1": "testval3"},
    ]
    compare_dicts_patch.side_effect = [False, True, False]
    assert oci_common_utils.is_in_list(test_list, {"testkey1": "testval2"}) is True
    assert compare_dicts_patch.call_count == 3

    compare_dicts_patch.reset_mock()
    ...
    compare_dicts_patch.side_effect = [False, False, False]
    ...
    test_list = []
    compare_dicts_patch.side_effect = []
    assert oci_common_utils.is_in_list(test_list, {"testkey2": "testval4"}) is False
    assert compare_dicts_patch.call_count == 0


def test_is_in_list_when_element_is_list(compare_lists_patch):
    test_list = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    compare_lists_patch.side_effect = [False, True, False]
    assert oci_common_utils.is_in_list(test_list, [4, 5, 6]) is True
    assert compare_lists_patch.call_count == 3

    compare_lists_patch.reset_mock()
    ...
    test_list = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    compare_lists_patch.side_effect = [False, False, False]
    ...
    test_list = []
    compare_lists_patch.side_effect = []
    assert oci_common_utils.is_in_list(test_list, [4, 5, 6]) is False
    assert compare_lists_patch.call_count == 0
