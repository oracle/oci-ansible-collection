...
class BootVolumeHelperCustom:
    def create_resource(self):
        ...
        boot_volume = super(BootVolumeHelperCustom, self).create_resource()
        create_time_taken = int(time.time() - create_start_time)
        # wait until the boot volume copy is done.
        boot_volume_get_response = self.get_get_fn()(boot_volume_id=boot_volume.id)
        return oci.wait_until(
            self.client,
            boot_volume_get_response,
            "is_hydrated",
            True,
