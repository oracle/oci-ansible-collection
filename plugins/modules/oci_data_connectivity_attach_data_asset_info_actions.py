...
DOCUMENTATION = """
---
module: oci_data_connectivity_attach_data_asset_info_actions
short_description: Perform actions on an AttachDataAssetInfo resource in Oracle Cloud Infrastructure
description:
    - Perform actions on an AttachDataAssetInfo resource in Oracle Cloud Infrastructure
    - For I(action=create_attach_data_asset), attaches a list of data assets to the given endpoint.
version_added: "2.9.0"
...
options:
    ...
    action:
        description:
            - The action to perform on the AttachDataAssetInfo.
        type: str
        required: true
        choices:
            - "create_attach_data_asset"
...
RETURN = """
attach_data_asset_info:
    description:
        - Details of the AttachDataAssetInfo resource acted upon by the current operation
    returned: on success
    type: complex
    contains:
        reference_info:
...
try:
    ...
    from oci.data_connectivity.models import CreateAttachDataAssetDetails
...
class AttachDataAssetInfoActionsHelperGen(OCIActionsHelperBase):
    """
    Supported actions:
        create_attach_data_asset
    """
    ...
    def create_attach_data_asset(self):
        action_details = oci_common_utils.convert_input_data_to_model_class(
            self.module.params, CreateAttachDataAssetDetails
        )
        return oci_wait_utils.call_and_wait(
            call_fn=self.client.create_attach_data_asset,
            call_fn_args=(),
            call_fn_kwargs=dict(
                registry_id=self.module.params.get("registry_id"),
                endpoint_id=self.module.params.get("endpoint_id"),
                create_attach_data_asset_details=action_details,
            ),
...
AttachDataAssetInfoActionsHelperCustom = get_custom_class(
    "AttachDataAssetInfoActionsHelperCustom"
)


class ResourceHelper(
    AttachDataAssetInfoActionsHelperCustom, AttachDataAssetInfoActionsHelperGen
):
    pass
...
def main():
    ...
    module_args.update(
        dict(
            ...
            action=dict(
                type="str", required=True, choices=["create_attach_data_asset"]
            ),
        )
    )
    ...
    resource_helper = ResourceHelper(
        module=module,
        resource_type="attach_data_asset_info",
