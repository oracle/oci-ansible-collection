...
DOCUMENTATION = """
---
module: oci_data_connectivity_data_preview_actions
short_description: Perform actions on a DataPreview resource in Oracle Cloud Infrastructure
description:
    - Perform actions on a DataPreview resource in Oracle Cloud Infrastructure
    - For I(action=create), provide data preview on live schema.
version_added: "2.9.0"
...
options:
    ...
    action:
        description:
            - The action to perform on the DataPreview.
        type: str
        required: true
        choices:
            - "create"
...
EXAMPLES = """
- name: Perform action create on data_preview
  oci_data_connectivity_data_preview_actions:
    # required
...
data_preview:
    description:
        - Details of the DataPreview resource acted upon by the current operation
    returned: on success
    type: complex
    contains:
        entity_name:
...
try:
    ...
    from oci.data_connectivity.models import CreateDataPreviewDetails
...
class DataPreviewActionsHelperGen(OCIActionsHelperBase):
    """
    Supported actions:
        create
    """
    ...
    def create(self):
        action_details = oci_common_utils.convert_input_data_to_model_class(
            self.module.params, CreateDataPreviewDetails
        )
        return oci_wait_utils.call_and_wait(
            call_fn=self.client.create_data_preview,
            call_fn_args=(),
            call_fn_kwargs=dict(
                registry_id=self.module.params.get("registry_id"),
                create_data_preview_details=action_details,
...
DataPreviewActionsHelperCustom = get_custom_class("DataPreviewActionsHelperCustom")


class ResourceHelper(DataPreviewActionsHelperCustom, DataPreviewActionsHelperGen):
    pass
