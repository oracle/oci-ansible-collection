...
short_description: Perform actions on a WaasPolicy resource in Oracle Cloud Infrastructure
description:
    - Perform actions on a WaasPolicy resource in Oracle Cloud Infrastructure
    - For I(action=accept_recommendations), accepts a list of recommended Web Application Firewall protection rules. Web Application Firewall protection rule
      recommendations are sets of rules generated by observed traffic patterns through the Web Application Firewall and are meant to optimize the Web
    ...
    resources:
        description:
            - "A resource to purge, specified by either a hostless absolute path starting with a single slash (Example: `/path/to/resource`) or by a relative
              path in which the first component will be interpreted as a domain protected by the WAAS policy (Example: `example.com/path/to/resource`)."
            - Applicable only for I(action=purge_cache).
        type: list
    action:
        description:
