...
DOCUMENTATION = """
---
module: oci_network_internet_gateway_actions
short_description: Perform actions on an InternetGateway resource in Oracle Cloud Infrastructure
description:
    - Perform actions on an InternetGateway resource in Oracle Cloud Infrastructure
    - For I(action=change_compartment), moves an internet gateway into a different compartment within the same tenancy. For information
...
options:
    ...
    action:
        description:
            - The action to perform on the InternetGateway.
        type: str
        required: true
        choices:
            - "change_compartment"
...
try:
    ...
    from oci.core.models import ChangeInternetGatewayCompartmentDetails
...
class InternetGatewayActionsHelperGen(OCIActionsHelperBase):
    """
    Supported actions:
        change_compartment
    """
    ...
    def change_compartment(self):
        action_details = oci_common_utils.convert_input_data_to_model_class(
            self.module.params, ChangeInternetGatewayCompartmentDetails
        )
        return oci_wait_utils.call_and_wait(
            call_fn=self.client.change_internet_gateway_compartment,
            call_fn_args=(),
            call_fn_kwargs=dict(
                ig_id=self.module.params.get("ig_id"),
                change_internet_gateway_compartment_details=action_details,
            ),
...
InternetGatewayActionsHelperCustom = get_custom_class(
    "InternetGatewayActionsHelperCustom"
)


class ResourceHelper(
    InternetGatewayActionsHelperCustom, InternetGatewayActionsHelperGen
):
    pass
...
def main():
    ...
    module_args.update(
        dict(
            ig_id=dict(aliases=["id"], type="str", required=True),
