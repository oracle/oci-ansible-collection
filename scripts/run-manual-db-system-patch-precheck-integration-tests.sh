...
echo "Running lengthy-integration tests for $CI_COMMIT_REF_NAME in $(pwd)"

function run_lengthy_integration_tests {
  for i in $(find ansible/test/integration/targets/ -type d -name oci* | grep oci_db_system-manual_update_lengthy_version_precheck)
