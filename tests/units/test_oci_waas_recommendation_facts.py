...
from ansible.modules.cloud.oracle import oci_waas_recommendation_facts
...
try:
    ...
    raise SkipTest("test_oci_waas_recommendation_facts.py requires `oci` module")
...
def test_list_recommendations_raises_service_error(
    ...
    with pytest.raises(ServiceError) as exc_info:
        oci_waas_recommendation_facts.list_recommendations(
            waas_client,
...
def test_list_recommendations_calls_list_all_resources_with_correct_parameters(
    waas_client, list_all_resources_patch
):
    waas_policy_id = "ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx"
    module = get_module(waas_policy_id=waas_policy_id, recommended_action="BLOCK")
    list_all_resources_patch.return_value = []
    oci_waas_recommendation_facts.list_recommendations(waas_client, module)
