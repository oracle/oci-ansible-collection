...
from ansible.modules.cloud.oracle import oci_waas_policy_facts
...
try:
    ...
    from oci.waas.models import WaasPolicySummary
except ImportError:
    raise SkipTest("test_oci_waas_policy_facts.py requires `oci` module")
...
def get_waas_policies():
    return [
        WaasPolicySummary(
            id="ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx1", display_name="testname1"
        ),
        WaasPolicySummary(
            id="ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx2", display_name="testname2"
        ),
    ]
...
def test_list_waas_policies_raises_service_error(waas_client, list_all_resources_patch):
    list_all_resources_patch.side_effect = ServiceError(
        500, "InternalServerError", dict(), "Internal Server Error"
    )
    with pytest.raises(ServiceError) as exc_info:
        oci_waas_policy_facts.list_waas_policies(
            waas_client,
...
def test_list_waas_policies_calls_list_all_resources_with_correct_parameters(
    waas_client, list_all_resources_patch
):
    id = [
        "ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx1",
        "ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx2",
    ]
    ...
    oci_waas_policy_facts.list_waas_policies(waas_client, module)
...
def test_list_waas_policies_calls_get_waas_policy(
    ...
    waas_policies = get_waas_policies()
    list_all_resources_patch.return_value = waas_policies
    oci_waas_policy_facts.list_waas_policies(waas_client, module)
    list_all_resources_patch.assert_called_once()
    assert call_with_backoff_patch.call_count == 2
    call_with_backoff_patch.assert_any_call(
        waas_client.get_waas_policy, waas_policy_id=waas_policies[0].id
    )
    call_with_backoff_patch.assert_any_call(
        waas_client.get_waas_policy, waas_policy_id=waas_policies[1].id
    )


def test_get_waas_policy_raises_service_error(waas_client, call_with_backoff_patch):
    call_with_backoff_patch.side_effect = ServiceError(
        500, "InternalServerError", dict(), "Internal Server Error"
    )
    with pytest.raises(ServiceError) as exc_info:
        oci_waas_policy_facts.get_waas_policy(
            waas_client,
            get_module(waas_policy_id="ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx"),
        )
    se = exc_info.value
    assert se.status == 500
    assert se.code == "InternalServerError"
...
def test_get_waas_policy_calls_call_with_backoff_with_correct_parameters(
    waas_client, call_with_backoff_patch
):
    waas_policy_id = "ocid1.waaspolicy.oc1..xxxxxEXAMPLExxxxx"
    module = get_module(waas_policy_id=waas_policy_id)
    call_with_backoff_patch.return_value = get_response(data=None)
    oci_waas_policy_facts.get_waas_policy(waas_client, module)
