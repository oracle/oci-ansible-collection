...
- For *state=present*, creates a new database in the specified Database Home. If the database version is provided, it must match the version of the Database Home. Applies to Exadata DB systems and Exadata Cloud at Customer.
- This resource has the following action operations in the :ref:`oci_database_actions <ansible_collections.oci_database_actions_module>` module: restore.
